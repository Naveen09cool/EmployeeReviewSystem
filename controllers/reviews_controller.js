...
module.exports.setReview = async function(req, res){
    ...
    console.log(`sdahdiuahadasfiaisfiafnajbfdao; ${receiver.assignedfrom.length}`);
    for(let i = 0; i<receiver.assignedfrom.length; i++){
        if(req.user){
            if(receiver.assignedfrom[i]== req.user.id){
